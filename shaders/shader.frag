...
float CalcOmniShadowFactor(PointLight light, int shadowIndex)
{
    ...
    float diskRadius = (1.0 + (viewDistance / omniShadowMap[shadowIndex].farPlane)) / 25.0;

    for(int i = 0; i < samples; i++)
    {
        float closest = texture(omniShadowMap[shadowIndex].shadowMap, fragToLight + sampleOffsetDirections[i] * diskRadius).r;
